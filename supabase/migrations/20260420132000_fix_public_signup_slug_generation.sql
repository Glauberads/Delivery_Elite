...
CREATE OR REPLACE FUNCTION public.slugify_tenant_name(value TEXT)
RETURNS TEXT
LANGUAGE sql
IMMUTABLE
AS $$
...
COMMENT ON FUNCTION public.slugify_tenant_name(TEXT)
IS 'Normaliza o nome da loja para slug: lowercase, sem acento, apenas a-z0-9 e hífen.';
...
BEGIN
    ...
    IF v_tenant_id IS NULL THEN
        v_base_slug := public.slugify_tenant_name(v_store_name);

        IF v_base_slug = '' THEN
            v_base_slug := public.slugify_tenant_name(split_part(v_email, '@', 1));
        END IF;
