-- Refactor RPC contract to avoid PostgREST ambiguity:
-- keep get_product_full() for authenticated flow
-- keep get_product_full_by_tenant(uuid) for public flow
-- remove overloaded get_product_full(uuid)
...
DROP FUNCTION IF EXISTS public.get_product_full_internal(uuid);

CREATE OR REPLACE FUNCTION public.get_product_full_internal(p_tenant_id UUID)
RETURNS TABLE (
    id UUID,
    name TEXT,
    description TEXT,
...
BEGIN
    ...
    RETURN QUERY
    SELECT *
    FROM public.get_product_full_internal(v_tenant_id);
END;
$$;
...
BEGIN
    RETURN QUERY
    SELECT *
    FROM public.get_product_full_internal(p_tenant_id);
END;
$$;
...
BEGIN
    ...
    RETURN QUERY
    SELECT *
    FROM public.get_product_full_internal(v_tenant_id) pf
    WHERE pf.id = p_product_id;
END;
$$;
